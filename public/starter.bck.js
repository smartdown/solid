...
/* global smartdownRelativeCardLoader */
...
function starter(basePrefix, doneHandler) {
  ...
  var outputDivSelector = '.smartdown-outer-container .smartdown-container';
  var postLoadMutator = null;
  var relativeCardLoaderOverride = null;
  var adjustHash = true;
  var media = {
  ...
  if (typeof smartdownRelativeCardLoaderOverride === 'function') {
    relativeCardLoaderOverride = smartdownRelativeCardLoaderOverride;
  }
  ...
  function relativeCardLoader(cardKey, outputDivId) {
    if (relativeCardLoaderOverride) {
      relativeCardLoaderOverride(cardKey, outputDivId);
      return;
    }
    ...
    if (part) {
      var output = document.querySelector(outputDivSelector);
  ...
  function loadHome(baseHash) {
    ...
    const outputDiv = document.querySelector(outputDivSelector);
    if (outputDiv) {
      console.log('locationHashChanged no outputDiv', outputDivSelector);
    }
    else {
      console.log('locationHashChanged outputDiv', outputDiv);
      relativeCardLoader(hash, outputDiv.id);
    }
  }
  ...
  function locationHashChanged() {
    console.log('locationHashChanged', outputDivSelector, window.location.hash, window.location.pathname, inhibitHash);
    var hash = window.location.hash;
    var args = '';
    var argsPos = hash.indexOf('?');
    if (argsPos >= 0) {
    ...
    if (inhibitHash === hash) {
      ...
      console.log('outputDivSelector', outputDivSelector, document.querySelectorAll(outputDivSelector));
